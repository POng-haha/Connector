...
func GenerateELKLogMain(c *gin.Context, timesRequest time.Time, request interface{}, response interface{}, appErr *app_error.AppError, serviceName string, userToken string, userRef string) string {
	timestamp := time.Now()
...
func GenerateELKLogLine(c *gin.Context, timesRequest time.Time, request interface{}, response interface{}, appErr *app_error.AppError, apikey string, endpoint string, serviceNameMain string, serviceNameLine string, userToken string, userRef string) string {
	timestamp := time.Now()
	formattedCurrentTimestamp := timestamp.Format("2006-01-02 15:04:05.000")
	formattedRequestTimestamp := formattedCurrentTimestamp
...
type HandleErrorResponse func(c *gin.Context, appErr *app_error.AppError)
