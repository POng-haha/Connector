...
func mustAtoi(s string) int {
    ...
    if err != nil {
        return 0 // หรือจัดการ error ตามต้องการ
    }
    return i
}
...
func FormatCheckRegisterRequest(checkRegisterReq domain.CheckRegisterRequest) string {
	...
	MobileNo        := utils.PadIntWithZero(mustAtoi(checkRegisterReq.MobileNo), 10)
	AgreementNo     := utils.PadIntWithZero(mustAtoi(checkRegisterReq.AgreementNo), 16)
