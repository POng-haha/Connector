...
func main() {
	...
	customer_lowerService := service_core.NewCustomer_lowerService(cfg, appLogger, tcpClient, dr.Routes, dr.Destinations)
	...
	customer_lowerHandler := handler_adapter.NewCustomer_lowerHandler(customer_lowerService, appLogger, apiKeyRepo, cfg)
	...
	router := handler_adapter.SetupRouter(appLogger, apiKeyRepo, collectionHandler, agreementHandler, creditcardHandler, commonHandler, selfServiceHandler, registerHandler, customer_lowerHandler, consentHandler, uhpHandler)
