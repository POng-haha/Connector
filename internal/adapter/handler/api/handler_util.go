...
func handleErrorResponse(c *gin.Context, appErr *app_error.AppError) {
	...
	switch appErr.Code {
	case app_error.ErrValidation.Code, app_error.ErrRequiedParam.Code, app_error.ErrApiChannel.Code, app_error.ErrApiRequestID.Code, app_error.ErrApiDeviceOS.Code, app_error.ErrIDCardNotFound.Code,
		 app_error.ErrSUEInfoNotFound.Code, app_error.ErrAgmNoNotFound.Code, app_error.ErrService.Code, app_error.ErrSystemI.Code, app_error.ErrSystemIUnexpect.Code:
...
func (h *CollectionHandler) validateHeadersAndAuth(c *gin.Context, method string, path string) *app_error.AppError {
	headers := getAPIHeaders(c)

	if !h.apikey.Validate(headers.APIKey, method, path) {
		h.logger.Warnw("Authorization failed", "path", path, "apiKey", headers.APIKey)
...
func (h *CollectionHandler) HandleValidationError(err error) *app_error.AppError {
